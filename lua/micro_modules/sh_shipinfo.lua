...
if SERVER then
	...
	hook.easy("InitPostEntity",function()
		...
		for i,origin_ent in pairs(ents.FindByName("micro_ship_*")) do
			local new_ship_info = {}
			new_ship_info.origin = origin_ent:GetPos()

			local function doTrace(dir)
				local tr = util.TraceLine{new_ship_info.origin, endpos = new_ship_info.origin+dir*10000}
				return tr.HitPos
			end
			...
			ship_ent:SetInternalOrigin(new_ship_info.origin)
			ship_ent:SetColor(MICRO_TEAM_COLORS[i])
			ship_ent:Spawn()
			ship_ent.home = home
			ship_ent.info = new_ship_info

			new_ship_info.ent_id = ship_ent:EntIndex()
		...
		for i,info in pairs(MICRO_SHIP_INFO) do
			...
			spawnInterior(Entity(info.ent_id),info.origin,ship_design)
		end

		PrintTable(MICRO_SHIP_INFO)
	end)
end
